...
# Ruta al archivo del panel
PANEL_ARCHIVE="/path/del/script/panel-banyio.tar.bz2"  # Cambia esto si lo mueves
...
if [ -f "$PANEL_ARCHIVE" ]; then
    ...
    # Reiniciar el panel XFCE para aplicar los cambios (en la sesión gráfica del usuario)
    sudo -u "$REAL_USER" DISPLAY=:0 DBUS_SESSION_BUS_ADDRESS="unix:path=/run/user/$(id -u "$REAL_USER")/bus" xfce4-panel -r &>/dev/null &
